...
---
##Selecting Clusters for PCA, UMAP and PHATE Embedding
...
new_clustertoPLot <- subset(aligned_rat_merged_subcluster, idents =  clusters_toPLot)
...
new_clustertoPLot$seurat_clusters <- factor(x = Idents(aligned_rat_merged_subcluster), levels = new_order)
...
# Assign the new order to the "seurat_clusters" factor variable in the new_clustertoPLot object
new_clustertoPLot$sub.clusters <- factor(x = Idents(aligned_rat_merged_subcluster), levels = new_order)
# Plot with cluster labels and specific color palette
