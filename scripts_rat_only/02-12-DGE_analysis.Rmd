...
aligned_rat_merged$sub.clusters <- factor(x =aligned_rat_merged$sub.clusters , levels = new_order)
```
...
rat_merged_subcluster_markers <- FindAllMarkers(aligned_rat_merged, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
...
```{r}
# Find all markers in the aligned_rat_merged object but using the DESeq2 test
all_markers <-  FindAllMarkers(aligned_rat_merged, min.pct = 0.25, test.use = "DESeq2",  verbose = T)
